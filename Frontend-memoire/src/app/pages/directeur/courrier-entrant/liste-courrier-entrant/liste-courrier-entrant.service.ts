...
export class AdvanceTableEntrantService extends UnsubscribeOnDestroyAdapter {
  private readonly API_URL = 'http://127.0.0.1:8000/api/directeur/courrier-entrant';
  isTblLoading = true;
  ...
  getAllAdvanceTables(): void {
    console.log('Fetching courriers from API for directeur:', this.API_URL);
    ...
    this.subs.sink = this.httpClient
      ...
      .subscribe(
        (response) => {
          console.log('API response for directeur courriers entrants:', response);
          ...
          if (response && response.success) {
            ...
            console.log('Processed courriers for directeur:', courriers);
        ...
        (error: HttpErrorResponse) => {
          this.isTblLoading = false;
          console.error('API error in directeur getAllAdvanceTables:', error);
          console.error('Error status:', error.status);
          console.error('Error message:', error.message);
          
          if (error.status === 401) {
            console.warn('Accès non autorisé - Vérifiez les permissions du directeur');
          }
